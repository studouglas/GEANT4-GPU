...
\newcommand{\sd}[1]{\authornote{orange}{SD}{#1}}
...
Added descriptions for each test, spelling fixes, minor rewording & Stuart & 2016-04-25\\
...
\item A .csv (Excel) file will be created to easily compare runtimes of each function with CUDA enabled and disabled.
\end{enumerate}
...
\subsection{void Init(istream \& aDataFile, G4int total, G4double ux, G4double uy)} 
	...
	\subsubsection{Test Inputs}
		...
		\toprule
		\multirow{2}{*}{\bf Test \#}  & \multicolumn{4}{c}{\bf Inputs} & \multirow{2}{*}{\bf Description}\\
		& \bf \texttt{aDataFile} & \bf \texttt{G4int} & \bf \texttt{ux} & \bf \texttt{uy}\\\midrule
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{Init_0} & Current data file & n & 1 & 1 & Runs initialize function with data file for current \texttt{vec} object\\
		\bottomrule
		\end{tabular}
		\end{table}
...
\subsection{G4ParticleHPVector \& operator = (const G4ParticleHPVector \& right)}
	Create a new, temporary G4ParticleHPVector object and assign the current vector to it. This operator is overridden, and uses a special GPU kernel function to perform the copying of the vector data from GPU memory of the \texttt{right} parameter to GPU memory of our new temporary vector. We need to ensure that the data vectors are properly allocated and copied to the new vector. Test outputs the data and the integral from the new vector and verifies that they are the same.
	...
	\subsubsection{Test Inputs}
		...
		\begin{tabular}{cll}
		\toprule
		\multirow{2}{*}{\bf Test \#}  & \multicolumn{1}{c}{\bf Inputs} & \multirow{2}{*}{\bf Description}\\
		& \bf \texttt{right}\\\midrule
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{OperatorEquals_0} & Current vector & Tests assignment of current vector to new vector\\
		\bottomrule
		\end{tabular}
		\end{table}	
...
\sd{Added more thorough description}
...
\subsection{const G4ParticleHPDataPoint GetPoint(G4int i)}
	...
	\subsubsection{Test Inputs}
		...
		\begin{tabular}{lll}
		\toprule
		\multirow{2}{*}{\bf Test \#}  & {\bf Inputs} & \multirow{2}{*}{\bf Description}\\
		& \bf \texttt{i} & \\\midrule
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{GetPoint_0} & -1 & \multirow{5}{*}{Verifies that getting a point at \texttt{i} returns the proper data point}\\
...
\subsection{G4double GetX(G4int i)}
	...
	\subsubsection{Test Inputs}
		...
		\multirow{2}{*}{\bf Test \#}  & \multicolumn{1}{c}{\bf Inputs} & \multirow{2}{*}{\bf Description}\\
		& \bf \texttt{i}\\\midrule
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{GetX_0} & -1 & \multirow{5}{*}{Verifies getting x value at \texttt{i} is correct}\\
...
\subsection{G4double GetEnergy(G4int i)} % Test cases finished
	...
	\subsubsection{Test Inputs}
		...
		\multirow{2}{*}{\bf Test \#}  & \multicolumn{1}{c}{\bf Inputs} & \multirow{2}{*}{\bf Description}\\
		& \bf \texttt{i}\\\midrule
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{GetEnergy_0} & -1 & \multirow{5}{*}{Verifies getting x value at \texttt{i} is correct}\\
...
\subsection{G4double GetY(G4int i)}
	...
	\subsubsection{Test Inputs}
		...
		\multirow{2}{*}{\bf Test \#}  & \multicolumn{1}{c}{\bf Inputs} & \multirow{2}{*}{\bf Description}\\
		& \bf \texttt{i}\\\midrule
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{GetY_0} & -1 & \multirow{5}{*}{Verifies getting y value at \texttt{i} is correct}\\
...
\subsection{G4double GetXsec(G4int i)} % Test cases finished
	...
	\subsubsection{Test Inputs}
		...
		\multirow{2}{*}{\bf Test \#}  & \multicolumn{1}{c}{\bf Inputs} & \multirow{2}{*}{\bf Description}\\
		& \bf \texttt{i}\\\midrule
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{GetXsec_0} & -1 & \multirow{5}{*}{Verifies getting y value at \texttt{i} is correct}\\
...
\subsection{void SetData(G4int i, G4double x, G4double y)}
	...
	Commas denote multiple sub-test inputs. If one of the sub-tests fail then the whole test fails.
		...
		\toprule
		\multirow{2}{*}{\bf Test \#}  & \multicolumn{3}{c}{\bf Inputs} & \multirow{2}{*}{\bf Description}\\
		& \bf \texttt{i} & \bf \texttt{x} & \bf \texttt{y}\\\midrule
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{SetData_0} & -1 & r1, r2, r3, r4, r5 & r1, r2, r3, r4, r5 & \multirow{5}{*}{tests setting point \texttt{i} to (x,y)}\\
...
\subsection{void SetPoint(G4int i, const G4ParticleHPDataPoint \& it)}	
	...
	\subsubsection{Test Inputs}
		...
		\begin{tabular}{llll}
		\toprule
		\multirow{2}{*}{\bf Test \#} & \multicolumn{2}{c}{\bf Inputs} & \multirow{2}{*}{\bf Description}\\
		& \texttt{i} & \texttt{it}\\\midrule
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{SetPoint_0} & -1 & rPoint, nPoint, zPoint & \multirow{5}{*}{Tests setting point \texttt{i} to each input individually}\\
...
\subsection{void SetX(G4int i, G4double e)}
	...
	Commas denote multiple sub-test inputs. If one of the sub-tests fail then the whole test fails.
		...
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{SetX_0} & -1 & r1, r2, r3, r4, r5 & \multirow{5}{*}{Tests setting x value of point \texttt{i} to each \texttt{e}}\\
...
\subsection{void SetEnergy(G4int i, G4double e)}
	...
	Commas denote multiple sub-test inputs. If one of the sub-tests fail then the whole test fails.
		...
		\multirow{2}{*}{\bf Test \#}  & \multicolumn{2}{c}{\bf Inputs} & \multirow{2}{*}{\bf Description}\\
		& \bf \texttt{i} & \bf \texttt{e}\\\midrule
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{SetEnergy_0} & -1 & r1, r2, r3, r4, r5 & \multirow{5}{*}{Tests setting x value of point \texttt{i} to each \texttt{e} value independently}\\
...
\subsection{void SetXsec(G4int i, G4double e)}
	...
	Commas denote multiple sub-test inputs. If one of the sub-tests fail then the whole test fails.
		...
		\multirow{2}{*}{\bf Test \#}  & \multicolumn{2}{c}{\bf Inputs} & \multirow{2}{*}{\bf Description}\\
		& \bf \texttt{i} & \bf \texttt{e}\\\midrule
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{SetXsec_0} & -1 & r1, r2, r3, r4, r5 \multirow{5}{*}{Tests setting y value of point \texttt{i} to each \texttt{e} value independently}\\
...
\subsection{G4double Sample()}
	...
	\subsubsection{Test Inputs}
		...
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{Sample_0} & N/A & Tests correctness of sampling vector\\
		\bottomrule
		\end{tabular}
		\end{table}
...
\subsection{G4double SampleLin()}% Test Cases Done
	...
	\subsubsection{Test Inputs}
		...
		\multirow{2}{*}{\bf Test \#}  & \multicolumn{1}{c}{\bf Inputs} & \multirow{2}{*}{\bf Description}\\
		& \bf \texttt{N/A}\\\midrule
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{SampleLin_0} & N/A & Tests correctness of sampling vector with linear interpolation \\
		\bottomrule
		\end{tabular}
		\end{table}
...
\subsection{void Times(G4double factor)} % Test Cases done
	...
	\subsubsection{Test Inputs}
		...
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{Times_0} & r1 & \multirow{5}{*}{Tests multiplying vector by \texttt{factor}}\\
...
\subsection{void ThinOut(G4double precision)} % Test Cases done 
	...
	\subsubsection{Test Inputs}
		...
		\multirow{2}{*}{\bf Test \#}  & \multicolumn{1}{c}{\bf Inputs} & \multirow{2}{*}{\bf Description}\\
		& \bf \texttt{factor}\\\midrule
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{ThinOut_0} & r1 & \multirow{5}{*}{Tests removing values within \texttt{factor} of each other from vector}\\
...
\subsection{G4double GetXsec(G4double e)}
	...
	Commas denote multiple sub-test inputs. If one of the sub-tests fail then the whole test fails.
		...
		\multirow{2}{*}{\bf Test \#}  & \multicolumn{1}{c}{\bf Inputs} & \multirow{2}{*}{\bf Description}\\
		& \bf \texttt{e}  \\\midrule
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{GetXsec_e_0} & r1, r2, r3, r4, r5 & \multirow{5}{*}{Tests getting first y value of point with energy at least \texttt{e}} \\
...
\subsection{G4double GetXsec(G4double e, G4int min)}
	...
	Commas denote multiple sub-test inputs. If one of the sub-tests fail then the whole test fails.
		...
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{GetXsec_e_min_0} & r1, r2, r3, r4, r5 & -1 & \multirow{5}{*}{\parbox{6cm}{Tests getting first y value of point with energy at least \texttt{e}, starting at \texttt{min}}}\\
...
15\% of the last data point.
	...
	\subsubsection{Test Inputs}
		...
		\multirow{2}{*}{\bf Test \#}  & \multicolumn{1}{c}{\bf Inputs} & \multirow{2}{*}{\bf Description}\\
		& \bf \texttt{N/A}\\\midrule
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{Get15percentBorder_0} & N/A & Tests getting first integral within 15\% of last data point \\
		\bottomrule
		\end{tabular}
		\end{table}
...
50\% of the last data point.
	...
	\subsubsection{Test Inputs}
		...
		\multirow{2}{*}{\bf Test \#}  & \multicolumn{1}{c}{\bf Inputs} & \multirow{2}{*}{\bf Description}\\
		& \bf \texttt{N/A}\\\midrule
		\refstepcounter{TestCounter}\arabic{TestCounter}\label{Get50percentBorder_0} & N/A & Tests getting first integral within 50\% of last data point\\
		\bottomrule
		\end{tabular}
		\end{table}
...
\sd{Added description to each test in inputs section, test num in results table refs back to that table with description}
	\subsubsection{Performance}
		\begin{figure}[H]
    	\centering
...
\ref{Init_0} - \ref{Get50percentBorder_0} & Unit tests & G4ParticleHPVector\\
\ref{sys1} - \ref{sys5} & System tests & G4ParticleHPDataPoint, G4ParticleHPVector, CMake Files\\
\bottomrule
\end{longtable}
\end{center}
