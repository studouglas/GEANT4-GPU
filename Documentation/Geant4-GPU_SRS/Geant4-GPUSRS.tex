...
%CPU finishes execution & results of tracking all particles (in) output file is written to,
% terminal displays the results of the simulation(out) & This step is the final step before a run ends. Once the run ends it prints out all remaining relevant data to the terminal and then ends\\
\hline
\end{tabularx}
\end{table}
...
{ReqType_Functional}
{PUC_SimulatingParticles}
{Existing projects should not be affected by the new code.}
{Design Requirement, need to ensure that users can continue to use GEANT-4 as before.}
{Running an existing simulation should produce identical results.}
{High}
{Created September 29, 2015}
\ds{This requirement should be split. The default case should have its own requirement.}\mmp{split requirement into two}
\\\\
...
{RegType_Functional}
{PUC_SimulatingParticles}
{If the user does not specify Geant4 to run on the GPU then it shall run on the CPU by default.}
{User should be able to use Geant4 with the GPU acceleration the exact same way they used it before}
{Running Geant4 without specifying to use the GPU will not run any code on the GPU}
{Low}
{Created January 8, 2016}

%\requirement
%{Req_HardwareDetection}
%{Req_Type_Functional}
%{PUC_SimulatingParticles}
%{GEANT4-GPU should be able to detect if the computer has a compatible graphics card that can run the GPU code}
%{Don't want GEANT4 to try to run GPU code on a graphics card that is not compatible}
%{When building GEANT4-GPU, The system should inform the user if they have a compatible graphics card or not}
%{Medium}
%{Created January 8 2016}
...
%{ReqType_Functional}
%{PUC_SimulatingParticles}
%{Trying to enable GPU computing with a computer that does not have a compatible graphics card should be detected and not be allowed.}
...
%one for what to do based on hardware detected.}\mmp{split into two requirements}
\mmp{Removed for same reason }

%\ds{Will users be aware of whether or not their machine can run the code
%on the GPU?}\mmp{This was taken care of in requirement HardwareDetection}
...
%{ReqType_Look}
%{PUC_SimulatingParticles}
%{The visualizations of particle simulations will be rendered according to the data generated.}
%{The margin that the visualization changes (compared to the GPU) should be within the acceptable margin of error, according to  (requirement \ref{ReqType_Performance}).}
%{The visualizations of particle interactions should accurately depict the slight change in particle data generated by running sections of the simulation in parallel (on the GPU).}
%{Medium}
...
%{The software follows a rigorous scientific standard.}
...
%{Medium}
%{Created October 6, 2015}
%\ds{``Rigorous"} \mmp{fixed spelling}
...
%{A person with no disabilities and a knowledge on shell based operating systems should be able to run the software.}
%{Medium}
%{Created October 6, 2015}
%\ds{Bad fit criterion}\mmp{changed fit criterion}
...
{ReqType_Performance}
{PUC_SimulatingParticles}
{Decrease the time it takes to run a particle simulation on the GPU while maintaining output that is determined to be precise according to (requirement \ref{Req_Precision})}
...
{ReqType_Performance}
{PUC_SimulatingParticles}
{Results should be produced accurately, within a certain margin of error, whether simulation is run on CPU or GPU.}
...
{The results of a GPU run simulation should be close to the results of the simulation run on the CPU given that the inputs are the same. How close the results of the simulations are is influenced by the difference in the random numbers generated on the CPU and the GPU.}
{High}
{Created September 27, 2015}
...
{ReqType_Performance}
{PUC_simulatingParticles}
{If an error occurs, the product will throw an exception with a description why the exception happened the same way Geant4 would throw an exception}
...
\mmp{added a requirement for safeguarding in case of an error}

\subsubsection{Capacity Requirements}
NA -- see requirement \ref{Req_SpeedLatency}.
%\requirement
%{Req_Capacity}
%{ReqType_Performance}
%{PUC_SimulatingParticles}
...
NA -- The product will be used by single researchers, and is run locally on their machines. This project does not encompass the functionality for distributing GEANT4-GPU to multiple GPU's. 

\ds{What if the researchers wanted to run it on a server with multiple GPUs
(or hundreds of CPU cores?)}\mmp{added a statement indicating this project does not encompass distributed system gpu functionality.}
...
{ReqType_Enviro}
{PUC_SimulatingParticles}
{The product shall work with GEANT-4 10.00.p02 and later (barring breaking changes in Geant4 introduced after the project's completion date).}
{Support for most recent versions is desireable to users.}
...
{ReqType_Enviro}
{PUC_SimulatingParticles}
{The product will be available on a public repository along with clear instructions for installation}
...
Because G4-STORK is open source, the code written can be maintained by anyone. The original developers will not be maintaining the code, it will have to be done by end users. 

\subsubsection{Supportability Requirements}
There will be a setup.txt file describing briefly how to add a function and set inital parameters. 

\subsubsection{Adaptability Requirements}
G4STORK is only able to be run on Fedora and OS X. Therefore the project is constrained to only being able to run on Fedora and OS X. Thus the completed project is expected to run on Fedora and OS X. 
\ds{Why only those?} \mmp{added explanation to why project will run only on Fedora and OSx}
...
NA -- There will be no compromise on the integrity of data outside of physical hardware malfunctions.
%\requirement
%{Req_Integrity}
%{ReqType_Security}
%{PUC_SimulatingParticles}
...
\vs{Talked about the effects of our design decisions on currently installed versions of Geant4.}
