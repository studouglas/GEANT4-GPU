...
\subsubsection{The Current Situation}
	The project is able to run all necessary simulations; it is a fork of GEANT-4 called G4-STORK (Geant4 STOchastic Reactor Kinetics). Currently, when the G4-STORK simulation is run with a large number of particles and collisions, The completed code will run all the same calculations, with more particles, and have the results in less time.
\subsubsection{The Context of The Work}
\begin{figure}[h]
\includegraphics[width=6in]{WorkContextDiagram} 
\end{figure} 
%\includepdf[pages={-},width={500pt},height={700pt}]{WorkContextDiagram.pdf}
%\includepdf[pages={-},width={500pt},height={700pt}]{WorkContextDiagram.pdf}
...
The written code should be well commented and appropriately spaced ensure that it is easily maintainable and modifiable. When code is changed in order to better parallelize it, the changes should be noted and also minimize (if possible) the impact on readability.
