...
\begin{itemize}
\item Try parallelizing other commonly use components in sim$
\begin{itemize}
\item Look for classes manipulating list-style data structur$
\item Classes with functions that have nested loops or are h$
\item Probabilistic functions and getter/setter functions wo$
\item Functions with conditional branching may cause bottlen$
\end{itemize}
\end{itemize}
\end{frame}
