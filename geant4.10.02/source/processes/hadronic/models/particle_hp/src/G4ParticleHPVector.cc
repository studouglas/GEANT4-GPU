...
G4ParticleHPVector & operator + (G4ParticleHPVector & left, G4ParticleHPVector & right)
{
    ...
    return *result;
    // return cudaVector->operatorPlus(left, right);
  #else
    G4ParticleHPVector * result = new G4ParticleHPVector;
    G4int j=0;
    G4double x;
...
{
  #if GEANT4_ENABLE_CUDA
    cudaVector = new G4ParticleHPVector_CUDA(n);
  #else
...
{ 
  #if GEANT4_ENABLE_CUDA
	 cudaVector->Merge(aScheme, aValue, &(*(active)->cudaVector), &(*(passive)->cudaVector));
  #else
    CleanUp();
...
{
  #if GEANT4_ENABLE_CUDA
    return cudaVector->IsBlocked(aX);
  #else
  	G4bool result = false;
...
G4double G4ParticleHPVector::Sample() // Samples X according to distribution Y
{
    ...
    // NOTE: theBuffered is private and never assigned to, so I commented it out
    // 
...
{    
  #if GEANT4_ENABLE_CUDA
    return cudaVector->Get15percentBorder();
  #else
...
{    
  #if GEANT4_ENABLE_CUDA
    return cudaVector->Get50percentBorder();
  #else
