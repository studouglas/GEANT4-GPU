 ...
 *                                                          MEM_free
 * This code is intended to manage memory better than malloc for
 *   the specific TOP-C data structures.  It's not worthwhile now,
 *   and so is not used, but maybe as the CPU-memory gap continues to grow ....
...
COMM_rank(), mem_hdr_head, mem_hdr_head->next, mem_hdr_tail);
  printf("rank: %d; owner: %d, free msg: %x, size: %d, source: %d, tag: %d"
         ", id: %d\n",
         COMM_rank(), MEM_buf_hdr(buf)->owner, buf, MEM_buf_size(buf),
