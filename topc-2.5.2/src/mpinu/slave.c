...
static int RECV(s, buf, len, flags)
int s;
char *buf;
size_t len;
int flags;
{ int size = 0;
  ...
  int count = 0;
  while ( len > size ) {
    CALL_CHK( tmp = recv, (s, buf + size, len - size, flags) );
    if ( tmp < 0 ) {
      printf("MPINU_mpi_slave:  bad receive\n"); fflush(stdout); exit(1);
    }
    size += tmp;
    count++;
    if (count > 100) {
      printf("size only %d, need %ul\n", size, len); fflush(stdout); exit(1);
    }
  }
  return size;
}
...
char *port;             /* port to connect to        */
{
  ...
  for ( i=0; i <= MPINU_num_slaves; i++ ) {
    ...
    if (size != sizeof(struct sockaddr_in)) {
      printf("MPINU_mpi_slave: Incomplete message received at index %d.\n", i);
      exit(1);
    }
  }
#ifdef DEBUG
for (i=0;i<=MPINU_num_slaves;i++) {
  ...
  { INT buf[1];
    ...
    for( size = 0; size == 0; )
      CALL_CHK( size = recv, (sd, (char *)buf, sizeof(INT), 0) );
