 ...
 **             In this version, continue task until band is fully reduced
 **                    or until there was no more work to do in REDO
...
struct band_info {
  ...
  int first_unred_col;
  /* Next two fields support decisions whether to REDO */
  int do_update;
  int prev_unred_col;
...
TOPC_BUF generate_task_input() {
  ...
  for (i = first_unred_band; i < b ; i++)
    /* First condition checks that last reduced band can be used to reduce
       band number i;  Otherwise, delay starting this task */
...
{ int band_no = *band_ptr;
  ...
  /* If REDO, intervening UPDATE changed matrix[] but not for our band_no */
  if (!TOPC_is_REDO()) {
...
#  endif
  ...
  task_out->do_update = (task_out->first_unred_band > band_no);
  if (TOPC_is_REDO()
      && task_out->prev_unred_col == first_unred_col[max(first_unred_band-1,0)])
  task_out->do_update = 0;
  task_out->prev_unred_col = first_unred_col[max(first_unred_band-1,0)];
...
{ 
  if (output_ptr->do_update) return UPDATE;
   else return REDO;
}
