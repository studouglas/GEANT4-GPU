 ...
 ** MODIFICATIONS FOR FINE-GRAINED SMP PARALLELISM:
 **   first_unred_band declared volatile, do_task places atomic read
 **   around it, and update_shared_data places atomic write around it.
...
volatile int first_unred_band = 0; /* Done when first_unred_band = b */
...
struct band_info {
   ...
   * malloc( sizeof(struct band_info_info)+BAND_SIZE * DIM*sizeof(float) )
   */
  float band[1];
};
 ...
 * task_out is a persistent thread-private variable, pointing to a
 * a malloc buffer that persists.  Ideally, if C allowed it, we would write:
 *    THREAD_PRIVATE struct band_info *task_out = NULL;
...
{ int band_no = *band_ptr;
  ...
  /* Added for use with fine-grain parallelism (atomic read/write) */
  int local_first_unred_band;
  /* Not all compilers support variable size arrays on stack */
...
#  endif
  ...
  /* On some SMP architectures, read of int is always atomic */
  TOPC_ATOMIC_READ(0) {
    local_first_unred_band = first_unred_band;
  }
  ...
  if (first_unred_col[band_no] <= first_unred_col[max(first_unred_band-1,0)]) {
    ...
    for (row = max(first_unred_col[band_no], 0);
         /* row is fully reduced */
         row < local_first_unred_band * band_size; row++) {/*row fully reduced*/
  ...
  task_out->first_unred_band =
    ( band_no == local_first_unred_band ? local_first_unred_band+1
      : local_first_unred_band );
...
{ float *mat_ptr = output_ptr->band;
  ...
  for (i = band_no*band_size*n; i < (band_no+1)*band_size*n; i++)
    ...
    if (output_ptr->first_unred_band > first_unred_band) {
      /* Notify slaves of new fully reduced band now */
      TOPC_ATOMIC_WRITE(0) {
